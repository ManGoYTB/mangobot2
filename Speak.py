...
class Speak:
    ...
    async def speak(self, ctx, channel):
        """Type -> m.speak vc <- For speak with staff ans at the end type -> exit"""
        ...
        if not channels:
            await self.bot.say("NOOOPE Type -> m.speak vc <- For speak with staff ans at the end type -> exit")
            return
        ...
        await self.bot.say("Now speak with staff"
                           "Type Hey to start\n"
        ...
        await self.bot.say("Communication closed.")
